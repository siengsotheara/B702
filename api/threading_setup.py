...
from api.bankingbot_api import FACEBOOK_TOKEN, CASA_LINK
from api.bankingbot_api import app, requests, data
from api.bankingbot_api import page, NotificationType
...
def start_payload_callback(payload, event):
	...
	print "debug: ", page.get_user_profile(sender_id)
	page.typing_on(sender_id)
	page.send(sender_id, "tester91" + ", welcome! Nice to see you here :). I'm PhillipBot. You can ask me with quick reply pop up below and you can either choose option in right menu.")
	d = json.dumps({
		"recipient":{
			"id":sender_id
		},
...
def click_contact_payload(payload, event):
...
# def click_saving_action(payload, event):
